...
public class SpawnsManager {
    ...
    private void loadConfig() {
        ...
        if (!config.contains("spawns")) return;
    ...
    public void deleteSpawn(int id) {
        if (id == 0) {
            spawn0 = null;
        } else {
            spawns.remove(id);
        ...
        configFile.getConfig().set("spawns." + id, null);
        configFile.saveConfig();
    }

    public void deleteAllSpawnsFromFile() {
        spawn0 = null;
        configFile.getConfig().set("spawns", null);
        configFile.saveConfig();
    }

    public Collection<Integer> getAllSpawnIds() {
        Set<Integer> keys = new HashSet<>(spawns.keySet());
        if (spawn0 != null) {
            keys.add(0);
        }
        return keys;
    ...
    public Map<Integer, Location> getAllSpawns() {
        Map<Integer, Location> all = new HashMap<>(spawns);
        if (spawn0 != null) {
            all.put(0, spawn0);
        }
        return all;
